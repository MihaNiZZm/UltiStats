...
data class BlockMarkerEvent(
    ...
    override val type: EventType = EventType.BLOCK_MARKER
            ...
            .map { stat ->
                when (stat.playerId) {
                    fromPlayer -> stat.copy(
                        attack = stat.attack.copy(dropsOnMarker = stat.attack.dropsOnMarker + value)
                    )
                    toPlayer -> stat.copy(
                        defense = stat.defense.copy(blocksAsMarker = stat.defense.blocksAsMarker + value)
                    )
                    else -> stat
                }
            }
